...
const ExchangeRate = (props: Props) => {
  let buyValue =
    props.rate['buy' + props.currency] === 0
      ? '-'
      : props.rate['buy' + props.currency];
  let sellValue =
    props.rate['sell' + props.currency] === 0
      ? '-'
      : props.rate['sell' + props.currency];

  let buyClassName =
    buyValue === props.best['buy' + props.currency] ? styles.BestBuy : null;
  let sellClassName =
    sellValue === props.best['sell' + props.currency] ? styles.BestSell : null;
  ...
  return (
    <tr
      ...
      onClick={props.selectPoint.bind(this, props.rate.id)}
    >
      <td className={styles.PointColumn}>
        <p className={styles.Title}>{props.rate.name}</p>
      ...
      <td className={styles.PhonesColumn}>{phones}</td>
      <td
        className={[buyClassName, styles.CurrencyCell, 'text-center'].join(' ')}
      >
        {buyValue}
      </td>
      <td
        className={[sellClassName, styles.CurrencyCell, 'text-center'].join(
          ' '
        )}
      >
        {sellValue}
